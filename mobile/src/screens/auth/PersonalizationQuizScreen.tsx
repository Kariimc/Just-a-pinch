...
export default function PersonalizationQuizScreen({ navigation }: Props) {
  ...
  async function finish() {
    // Build a profile even when none exists yet (e.g. email confirmation
    // pending) so quiz answers are never dropped.
    ...
    await saveProfile({ ...base, dietaryPrefs: dietary, skillLevel: skill, householdSize: household });
          ...
          {SKILLS.map(s => (
            ...
            >
              ...
              <View style={[styles.check, skill === s.key && styles.checkOn]}>
                {skill === s.key && <Icon name="check" size={14} color="#fff" />}
              </View>
            </TouchableOpacity>
          ))}
...
const styles = StyleSheet.create({
  ...
  stepTxt: { fontFamily: Fonts.uiBold, fontSize: 12.5, color: Colors.ink2 },
  skip: { fontFamily: Fonts.uiBold, fontSize: 13.5, color: Colors.accentDeep },
  ...
  title: { fontFamily: Fonts.displayMedium, fontSize: 24, color: Colors.ink, marginTop: 22, letterSpacing: -0.3 },
  ...
  label: { fontFamily: Fonts.uiSemiBold, fontSize: 13, color: Colors.ink2, marginBottom: 10, marginTop: 22 },
  ...
  skillTitle: { fontFamily: Fonts.uiBold, fontSize: 15, color: Colors.ink },
  skillSub: { fontFamily: Fonts.uiRegular, fontSize: 12.5, color: Colors.ink3, marginTop: 2 },
  ...
  stepperLabel: { fontFamily: Fonts.uiSemiBold, fontSize: 14.5, color: Colors.ink },
